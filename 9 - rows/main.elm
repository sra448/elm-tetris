...
removeFullRows ( model, msg ) =
    let
        ...
        ( scoreMultiplier, fallenTiles ) =
            List.foldl removeFullRowsReducer
                ( 0, Set.empty )
                (List.reverse rows)
    in
...
removeFullRowsReducer : Set Tile -> ( Int, Set Tile ) -> ( Int, Set Tile )
removeFullRowsReducer tiles ( removedRowsCount, remainingTiles ) =
    if Set.size tiles == boardWidth then
        ( removedRowsCount + 1, remainingTiles )
    else
        ( removedRowsCount
        , Set.union
            (Set.map
                (\( ( x, y ), color ) ->
                    ( ( x, y + removedRowsCount ), color )
                )
                tiles
            )
            remainingTiles
